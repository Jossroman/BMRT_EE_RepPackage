...
@#define Lag = 5
@#define All_Y = ["K","AL","Energy" ]
@#define All_D = ["K","Energy" ]
...
e_t_shock_1     $e_T$      (long_name='Temperature Innovation 1')
e_t_shock_2     $e_T$      (long_name='Temperature Innovation 2')
...
@#elseif habits == 1
[name='FOC H']
(1+g_a)*lambda*lambda_h  = BETTA*(M*lambda(+1)*lambda_h(+1) +GAMMA_H*(c_t(+1)-GAMMA_H*h)^(-SIGMA));
@#endif
...
@#for m in 1:Lag
       + discount_factor_@{m}* (v_Energy(@{m}) * TEMP_MEAN*D1_Energy_@{m} * y_Energy(@{m}))
@#endfor
) - (
@#for m in 1:Lag
       + discount_factor_@{m}* (v_K(@{m}) * TEMP_MEAN*D1_K_@{m} * y_K(@{m}))
@#endfor
) - (
@#for m in 1:Lag
...
@#for o in 2:m
     discount_factor_@{o} = discount_factor_@{o-1} * (BETTA*lambda(@{o})/lambda(@{o}-1));
@#endfor
...
[name='FOC K']
v_K = v_y_t*GAMMA_K*(y_K)^(-1/EPS_Y)*(y_t)^(1/EPS_Y)*(G_Y)^((EPS_Y-1)/EPS_Y);

[name='FOC Energy']
v_Energy = v_y_t*GAMMA_Energy*(y_Energy)^(-1/EPS_Y)*(y_t)^(1/EPS_Y)*(G_Y)^((EPS_Y-1)/EPS_Y) - phi_emission*v_emission;
...
@#for o in 2:m
     discount_factor_@{o} = discount_factor_@{o-1} * BETTA;
@#endfor
...
@#elseif climate_model == 1    
[v_t, v_x_tot, v_emission, v_fossil, v_x_box0, v_x_box1, v_x_box2, v_x_box3] = 
                                                get_ss_v_fossil_energy_hybrid(TEMP_MEAN,phi_emission,v_y_t,G_Y,v_AL,v_K,y_t,EPS_Y, y_AL,y_K, 
                                                                              D1_K_1, D1_K_2, D1_K_3, D1_K_4, D1_K_5, 
                                                                              D1_AL_1, D1_AL_2, D1_AL_3, D1_AL_4, D1_AL_5, 
                                                                              D1_Energy_1,D1_Energy_2 ,D1_Energy_3,D1_Energy_4,D1_Energy_5, 
                                                                              y_Energy, GAMMA_Energy, BETTA, ZETTA_1, ZETTA_2, 
                                                                              DELTA_X_BOX0,DELTA_X_BOX1,DELTA_X_BOX2,DELTA_X_BOX3, 
                                                                              XI_E_BOX0,XI_E_BOX1,XI_E_BOX2,XI_E_BOX3);
