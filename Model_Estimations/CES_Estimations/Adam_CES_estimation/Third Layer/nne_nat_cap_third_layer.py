...
def simulate_y(beta_og, sigma, gamma, x_o, x_g, x_c):
    """
    ...
    y_sim = np.log(gamma) + (0.4149 / rho) * np.log(ces)
...
def generate_training_data_NNE(L, alpha_param, beta_param, sigma_lb, sigma_ub, gamma_lb, gamma_ub, x_o, x_g, x_c):
    """
    Generate training data for Neural Network Estimation (NNE).
    Now we sample sigma instead of rho.
    """
    input_data = []
    labels = []
    ...
    for i in range(L):
        ...
        # Simulate output y
        y_sim = simulate_y(beta_og, sigma, gamma, x_o, x_g, x_c)
        ...
        labels.append([beta_og, sigma, gamma])
...
def main():
    ...
    data_file_path = r"C:\Users\adity\Dropbox\Adi-Simone-Ghassane\Natural Capital\CES ML Estimation\matlab_data_fossil.xlsx"
    ...
    # For demonstration, let's pick sigma in [1.1, 1.5] (just as an example)
    # Ensure this aligns with your theoretical priors.
    sigma_lb = 1.1
    sigma_ub = 1.5
    ...
    param_names = ['beta_og', 'sigma', 'gamma']
