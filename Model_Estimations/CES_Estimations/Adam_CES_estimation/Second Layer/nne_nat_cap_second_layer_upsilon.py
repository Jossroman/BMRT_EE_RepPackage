#nne_nat_cap_second_layer_upsilon.py
import os
...
import matplotlib.pyplot as plt  # Reintroduced for plotting
import random
...
# Set environment variables for deterministic TensorFlow behavior
...
def simulate_y(beta_foss, sigma, gamma, upsilon, F, R):
    """
    ...
    rho = (sigma - 1) / sigma
    if rho != 0:
        y_sim = np.log(gamma) + (upsilon / rho) * np.log(beta_foss * F ** rho + (1 - beta_foss) * R ** rho)
    else:
        y_sim = np.log(gamma) + beta_foss * np.log(F) + (1 - beta_foss) * np.log(R)
    return y_sim

def generate_training_data_NNE(L, alpha_param, beta_param, gamma_lb, gamma_ub, sigma_lb, sigma_ub, upsilon_lb, upsilon_ub, F_real, R_real):
    """
    ...
    for i in range(L):
        ...
        upsilon = uniform.rvs(loc=upsilon_lb, scale=upsilon_ub - upsilon_lb, random_state=rng)

        # Simulate output y
        y_sim = simulate_y(beta_foss, sigma, gamma, upsilon, F_real, R_real)
        ...
        labels.append([beta_foss, sigma, gamma, upsilon])
...
def build_model(input_shape):
    ...
    # Output for means
    means = Dense(4)(x)  # Updated to predict 4 means

    # Output for variances (before softplus)
    variances_pred = Dense(4)(x)  # Updated to predict 4 variances
...
def custom_loss(y_true, y_pred):
    ...
    mu = y_pred[:, :4]  # Updated for 4 parameters
    variances_pred = y_pred[:, 4:]  # Updated for 4 parameters
...
def estimate_parameters_NNE(model, observed_moments):
    ...
    estimated_means = estimated_output[0, :4]  # Updated for 4 parameters
    variances_predicted = estimated_output[0, 4:]  # Updated for 4 parameters
...
def main():
    ...
    sigma_lb = 4.8
    sigma_ub = 5.5
    upsilon_lb = 0.90   # Define lower bound for upsilon
    upsilon_ub = 1  # Define upper bound for upsilon
    ...
    param_names = ['beta_foss', 'sigma', 'gamma', 'upsilon']
    ...
    predicted_means_val = y_pred_val[:, :4]  # Updated for 4 parameters
    ...
    output_path = Path.cwd() / "second_layer_results_upsilon.xlsx"
