...
syms pk energy rho beta_1 beta_2 beta_3 gamma labour

ces = log(gamma) - (1/rho)*log(beta_1*pk^(-rho) + beta_2*energy^(-rho) + (1-beta_1 - beta_2)*labour^(-rho)); 

parameters = [beta_1, beta_2, beta_3, rho, gamma];
...
objective = @(params) 0.5*sqrt(sum((Y_d - ces_func(params(1), params(2), params(3), params(4), params(5), ...
    pk_d, energy_d, labour_d)).^2));

initial_guess = [0.4, 0.2, 0.4, -0.3, 0.333];
...
parfor i = 1:num_bootstrap
    ...
    objective_boot = @(params) 0.5*sqrt(sum((Y_d_boot - ces_func(params(1), params(2), params(3), params(4), params(5),...
...
first_layer_bootstrap_table = array2table(bootstrap_params, 'VariableNames', {'beta_1', 'beta_2', 'beta_3', 'rho', 'gamma'});
...
predicted_Y = ces_func(estimated_params(1), estimated_params(2), estimated_params(3), estimated_params(4), estimated_params(5),...
...
output_file = fullfile(cd, "first_layer_ene_prod_sqp.xlsx");
