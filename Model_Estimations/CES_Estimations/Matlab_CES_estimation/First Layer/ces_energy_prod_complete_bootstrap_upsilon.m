...
syms pk energy rho beta_1 beta_2 beta_3 gamma labour upsilon

ces = log(gamma) - (upsilon/rho)*log(beta_1*pk^(-rho) + beta_2*energy^(-rho) + (1-beta_1 - beta_2)*labour^(-rho)); 

parameters = [beta_1, beta_2, beta_3, rho, gamma, upsilon];
data_variables = [pk, energy, labour];
...
objective = @(params) 0.5*sqrt(sum((Y_d - ces_func(params(1), params(2), params(3), params(4), params(5), params(6), ...
    pk_d, energy_d, labour_d)).^2));

initial_guess = [0.4, 0.2, 0.4, -0.3, 0.333, 1];
...
parfor i = 1:num_bootstrap
    ...
    objective_boot = @(params) 0.5*sqrt(sum((Y_d_boot - ces_func(params(1), params(2), params(3), params(4), params(5), params(6),...
        pk_d_boot, energy_d_boot, labour_d_boot)).^2));
    
    % Optimize for bootstrapped data
    [bootstrap_params(i, :), ~] = fmincon(objective_boot, initial_guess, [], [], Aeq, beq, lb, ub, [], options);
end

first_layer_bootstrap_table = array2table(bootstrap_params, 'VariableNames', {'beta_1', 'beta_2', 'beta_3', 'rho', 'gamma', 'upsilon'});
...
predicted_Y = ces_func(estimated_params(1), estimated_params(2), estimated_params(3), estimated_params(4), estimated_params(5), estimated_params(6),...
    pk_d, energy_d, labour_d);
...
var_names = {'pk', 'energy', 'labour', 'rho', 'gamma', 'upsilon', 'MSE'};
...
output_file = fullfile(cd, "first_layer_ene_prod_upsilon_sqp.xlsx");
