...
syms pk ecosystem cropland min energy rho beta_1 beta_2 beta_3 beta_4 beta_5 gamma beta_6 labour

ces = log(gamma) - (1/rho)*log(beta_1*(pk)^(-rho) + beta_2*ecosystem^(-rho) + beta_3*cropland^(-rho) + beta_4*min^(-rho) + (beta_5)*energy^(-rho) + (1-beta_1 - beta_2 - beta_3 - beta_4 - beta_5)*labour^(-rho)); 

parameters = [beta_1, beta_2, beta_3, beta_4, beta_5, beta_6, rho, gamma];
...
objective = @(params) 0.5*sqrt(sum((Y_d - ces_func(params(1), params(2), params(3), params(4), params(5), params(6), params(7), params(8),...
    pk_d, ecosystem_d, cropland_d, min_d, energy_d, labour_d)).^2));

initial_guess = [.4, .05, .1, .05, .2, .1, -0.3, 0.333];

%upper and lower bounds
lb = [0, 0, 0, 0, 0, 0,-1, -Inf];
ub = [1, 1, 1, 1, 1, 1, Inf, Inf];
...
parfor i = 1:num_bootstrap
    ...
    objective_boot = @(params) 0.5*sqrt(sum((Y_d_boot - ces_func(params(1), params(2), params(3), params(4), params(5), params(6), params(7), params(8),...
...
first_layer_bootstrap_table = array2table(bootstrap_params, 'VariableNames', {'beta_1', 'beta_2', 'beta_3', 'beta_4', 'beta_5', 'beta_6', 'rho', 'gamma'});
...
predicted_Y = ces_func(estimated_params(1), estimated_params(2), estimated_params(3), estimated_params(4), estimated_params(5), estimated_params(6), estimated_params(7), estimated_params(8),...
...
output_file = fullfile(cd, "first_layer_sqp.xlsx");
