...
syms gamma_e beta_fe beta_re rho_e y_fe y_re upsilon

ces = log(gamma_e) - (upsilon/rho_e)*log(beta_fe*y_fe^(-rho_e) + (1 - beta_fe)*y_re^(-rho_e)); 


parameters = [beta_fe, beta_re, rho_e, gamma_e, upsilon];
...
objective = @(params) .5*sqrt((sum((Y_d - ces_func(params(1), params(2), params(3), params(4), params(5), ...
    y_fe_d, y_re_d)).^2)));

initial_guess = [0.5, 0.5, -.4, 4, 0.95];
...
disp('Estimated parameters: beta_fe beta_re rho_e gamma_e upsilon');
...
parfor i = 1:num_bootstrap
    ...
    objective_boot = @(params) .5*sqrt((sum((Y_d_boot - ces_func(params(1), params(2), params(3), params(4), params(5), ...
...
second_layer_bootstrap_table = array2table(bootstrap_params, 'VariableNames', {'beta_fe', 'beta_re', 'rho_e', 'gamma_e', 'upsilon'});
...
var_names = {'fossil_energy', 'renew_energy', 'rho_e', 'gamma_e', 'upsilon', 'MSE'};
...
output_file = fullfile(cd, "second_layer_upsilon_sqp.xlsx");
